...
router.get('/:noticeId/',verifyToken,normalAccess, async (req, res) => {
	try {
		...
		const [member, fields] = await conn.execute('SELECT members, message, onChain FROM application WHERE notice_id = ?', [params.noticeId]);
...
router.post('/:noticeId/regist',verifyToken,normalAccess, async (req, res) => {
	try {
		...
		const bind = [null, params.noticeId, token.id, JSON.stringify(member[0]), body.message, nowTime, nowTime, "0"]
